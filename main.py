#!/usr/bin/env python
"""
Main script to run gamma ray shielding simulations with simplified options.
"""
...
def run_full_simulations():
    """Run the full set of simulations from config parameters"""
...
def main():
    """Run a single gamma ray shielding simulation with specified parameters"""
    
    parser = argparse.ArgumentParser(description='Run gamma ray shielding simulation')
    parser.add_argument('--energy', type=float, default=1.0,
                        help='Energy of gamma ray source in MeV (default: 1.0)')
    parser.add_argument('--diameter', type=float, default=0.5,
                        help='Diameter of air channel in cm (default: 0.5)')
    parser.add_argument('--distance', type=float, default=50,
                        help='Distance from back of wall to detector in cm (default: 50)')
    parser.add_argument('--angle', type=float, default=0,
                        help='Angle of detector from central axis in degrees (default: 0)')
    parser.add_argument('--output', type=str, default='result.json',
                        help='Output file for simulation results (default: result.json)')
    parser.add_argument('--full', action='store_true',
                        help='Run the full set of simulations from config parameters')
    ...
    # If full flag is set, run all simulations
    if args.full:
        run_full_simulations()
        return
    
    # Otherwise run a single simulation with specified parameters
    print(f"Running simulation with: Energy={args.energy} MeV, Channel Diameter={args.diameter} cm, "
          f"Distance={args.distance} cm, Angle={args.angle}°")
    
    # Run the simulation
    result = run_simulation(args.energy, args.diameter, args.distance, args.angle)
    ...
    print(f"Channel Diameter: {args.diameter} cm")
    print(f"Detector Distance: {args.distance} cm")
    print(f"Detector Angle: {args.angle}°")
    print(f"Dose Rate: {dose_str} rem/hr")
    print(f"\nResults saved to: results/{args.output}")
